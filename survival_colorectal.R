setwd("F:\\Green University\\Personal\\Research\\Effects of Welding on Health\\Genetic Effect of Welding on the Cancer Diseases\\Survival\\Colorectal cancer")
#################################################################
##############################working with clinical data#########
#################################################################
dato<-read.csv("coadread_tcga_clinical_data.csv",header = TRUE,stringsAsFactors = FALSE)
...
expr_mr<-read.csv("data_mRNA_median_Zscores2.csv",header=TRUE,stringsAsFactors = FALSE)
